...
export default function QuizEditor() {
  ...
  const createQuizForCourse = async () => {
    if (!cid) return;
    const newQuiz = { cid, ...quiz };
    const localQuiz = await coursesClient.createQuizForCourse(cid, newQuiz);
    ...
    <div id="wd-quizzes-editor" className="d-flex flex-column">
        ...
        <div>
          <Link id="wd-save" type="button" onClick={() =>     {                  
                    ...
                    } className="btn btn-lg btn-danger float-end ms-1" to={"/Kanbas/Courses/"+ cid +"/Quizzes"}> Save </Link>
